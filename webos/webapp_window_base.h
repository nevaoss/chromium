...
class WEBOS_EXPORT WebAppWindowBase : public WebAppWindowDelegate,
 ...
 public:
  ...
  enum Resizable {
    RESIZE_NONE = 0,
    RESIZE_FIT_TO_DISPLAY = 1 << 0,
    RESIZE_ROTATE_DISPLAY = 1 << 1,

    RESIZE_DEFAULT = RESIZE_FIT_TO_DISPLAY | RESIZE_ROTATE_DISPLAY,
  };

  struct InitParams {
    int width;
    int height;
    unsigned int resizable = RESIZE_DEFAULT;
  };
  ...
  void SetFirstActivateTimeoutMs(int timeout);
  ...
  void SetInputRegion(const std::vector<Region>& regions);
  ...
  void InitWindow(int width, int height);  // For the backward-compatibility
  void InitWindowWithParams(InitParams params);
