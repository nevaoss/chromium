...
class BrowserShellWindow final
 ...
 public:
  ...
  cppgc::Persistent<gin::WeakCell<BrowserShellWindow>> GetAsWeak() {
    return gin::WrapPersistent(weak_factory_.GetWeakCell(
        v8::Isolate::GetCurrent()->GetCppHeap()->GetAllocationHandle()));
  }

  void Setup(const std::string& name, browser_shell::mojom::RectPtr bounds);

  // browser_shell::mojom::ShellWindowClient
  void OnBoundsChanged(browser_shell::mojom::RectPtr bounds) override;
 ...
 private:
  ...
  void GetBounds(gin::Arguments* args);
  ...
  mojo::AssociatedReceiver<browser_shell::mojom::ShellWindowClient>
      receiver_{this};
  v8::TracedReference<v8::Object> page_view_object_;
  std::map<uint64_t, v8::TracedReference<v8::Object>> child_view_objects_;
  gin::WeakCellFactory<BrowserShellWindow> weak_factory_{this};
};
