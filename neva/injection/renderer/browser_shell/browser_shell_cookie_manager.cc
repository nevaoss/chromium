...
gin::WrapperInfo BrowserShellCookieManager::kWrapperInfo = {
...
BrowserShellCookieManager::BrowserShellCookieManager(
  ...
  remote_->BindClient(base::BindOnce(&BrowserShellCookieManager::SetupClient,
...
bool BrowserShellCookieManager::GetAllCookiesForTesting(gin::Arguments* args) {
  ...
  remote_->GetAllCookiesForTesting(
      base::BindOnce(&BrowserShellCookieManager::OnGetAllCookiesResponse,
...
void BrowserShellCookieManager::OnGetAllCookiesResponse(
  ...
  if (gin::TryConvertToV8(isolate, cookie_list, &result)) {
    std::ignore = local_callback->Call(context, wrapper, /*argc =*/1, &result);
  }
}
