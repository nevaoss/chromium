...
namespace events {
const char kActivateContents[] = "activate-contents";
...
const char kOnTouchStartEvent[] = "touch-start";
const char kOnZoomFactorChanged[] = "zoomchange";
const char kPageTitleUpdated[] = "page-title-updated";
const char kPageContentsActivated[] = "page-contents-activated";
const char kPageContentsDeactivated[] = "page-contents-deactivated";
const char kOnCapturingMediaStreamChanged[] = "capture-media-stream";
...
void BrowserShellPageContents::CaptureVisibleRegion(gin::Arguments* args) {
  ...
  if (!args->GetData(&callback)) {
    ...
    v8::Local<v8::Value> output_width_value;
    if (details->Get(context, gin::StringToV8(isolate, "output_width"))
            .ToLocal(&output_width_value)) {
      if (!gin::Converter<int32_t>::FromV8(
            isolate, output_width_value, &output_width)) {
        output_width = 0;
      }
    }

    v8::Local<v8::Value> output_height_value;
    if (details->Get(context, gin::StringToV8(isolate, "output_height"))
            .ToLocal(&output_height_value)) {
      if (!gin::Converter<int32_t>::FromV8(
            isolate, output_height_value, &output_height)) {
        output_height = 0;
      }
    }
  ...
  remote_->CaptureVisibleRegion(
      format, quality, output_width, output_height,
...
void BrowserShellPageContents::ActivateContents() {
  DoEmit(events::kActivateContents);
}
...
void BrowserShellPageContents::OnCapturingMediaStreamChanged(
  ...
  event_info
      ->Set(context, gin::StringToV8(isolate, "state"),
            v8::Boolean::New(isolate, is_capturing_media_stream))
      .Check();

  DoEmit(events::kOnCapturingMediaStreamChanged, event_info);
}
...
void BrowserShellPageContents::OnAuthChallenge(
  ...
  auto* login_obj = cppgc::MakeGarbageCollected<BrowserShellLogin>(
      isolate->GetCppHeap()->GetAllocationHandle(), this, std::move(challenge));

  v8::Local<v8::Object> login_obj_wrapper;
  if (!login_obj->GetWrapper(isolate).ToLocal(&login_obj_wrapper)) {
    return;
  }

  DoEmit(events::kAuthChallengeEvent, login_obj_wrapper);
}
...
void BrowserShellPageContents::OnNewWindowOpen(
  ...
  window_info_v8
      ...
      .Check();
  window_info_v8
      ->Set(context, gin::StringToV8(isolate, "isRelatedSiteInstance"),
            v8::Boolean::New(isolate, window_info->related_site_instance))
      .Check();
  ...
  remote_->BindNewPageContentsById(
      ...
      base::BindOnce(&injections::BrowserShellPageContents::Setup,
                     new_page_contents->GetAsWeak()));

  v8::Local<v8::Object> new_page_contents_object;
  if (new_page_contents->GetWrapper(isolate).ToLocal(
          &new_page_contents_object)) {
    DoEmit(events::kOnNewWindowOpen, new_page_contents_object, window_info_v8);
  }
}
...
void BrowserShellPageContents::OnPageContentsActivated() {
  DoEmit(events::kPageContentsActivated);
}

void BrowserShellPageContents::OnPageContentsDeactivated(
    bool by_page_discarder) {
  DoEmit(events::kPageContentsDeactivated, by_page_discarder);
}
...
void BrowserShellPageContents::AckPermission(PermissionRequest::Decision result,
                                             uint64_t request_id,
                                             uint64_t) {
  if (remote_.is_bound()) {
    const bool ack = (result == PermissionRequest::Decision::kAllow) ||
                     (result == PermissionRequest::Decision::kAllowThisTime);
    remote_->AckPermission(ack, request_id);
  }
}
...
void BrowserShellPageContents::OnPermissionRequest(
  ...
  if (!GetListenerCount(events::kOnPermissionRequest)) {
    // id - request id, id_ - this page contents id
    AckPermission(PermissionRequest::Decision::kIgnore, id, id_);
    return;
  }
  ...
  auto* request_obj = cppgc::MakeGarbageCollected<PermissionRequest>(
      isolate->GetCppHeap()->GetAllocationHandle(), this, id, id_);
...
void BrowserShellPageContents::OnTouchStartEvent() {
   DoEmit(events::kOnTouchStartEvent);
}
...
void BrowserShellPageContents::RunJSDialog(const std::string& type,
  ...
  auto* dialog_controller = cppgc::MakeGarbageCollected<DialogController>(
      isolate->GetCppHeap()->GetAllocationHandle(), this);

  v8::Local<v8::Object> dialog_controller_wrapper;
  if (!dialog_controller->GetWrapper(isolate).ToLocal(
          &dialog_controller_wrapper)) {
    return;
  }

  DoEmit(events::kDialog, type, message, dialog_controller_wrapper);
}
...
void BrowserShellPageContents::Trace(cppgc::Visitor* visitor) const {
  visitor->Trace(weak_factory_);
  InjectionEventsEmitter<BrowserShellPageContents>::Trace(visitor);
  gin::Wrappable<BrowserShellPageContents>::Trace(visitor);
}
...
void BrowserShellPageContents::LoadFile(gin::Arguments* args) {
  ...
  if (GURL(file_path).is_valid()) {
    ...
    remote_->LoadURL(
        file_path,
        base::BindOnce(&BrowserShellPageContents::OnLoadURL, GetAsWeak()));
    return;
  }
  ...
  remote_->LoadURL(
      url, base::BindOnce(&BrowserShellPageContents::OnLoadURL, GetAsWeak()));
}
...
const gin::WrapperInfo* BrowserShellPageContents::wrapper_info() const {
  return &kWrapperInfo;
}
