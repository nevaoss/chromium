...
BrowserShellWebRequest::BrowserShellWebRequest(
  ...
  remote_->BindClient(
      base::BindOnce(&BrowserShellWebRequest::SetupClient, GetAsWeak()));
}
...
void BrowserShellWebRequest::Trace(cppgc::Visitor* visitor) const {
  visitor->Trace(weak_factory_);
  visitor->Trace(on_before_request_listener_);
  gin::Wrappable<BrowserShellWebRequest>::Trace(visitor);
}
...
gin::ObjectTemplateBuilder BrowserShellWebRequest::GetObjectTemplateBuilder(
    v8::Isolate* isolate) {
  return gin::Wrappable<BrowserShellWebRequest>::GetObjectTemplateBuilder(
             isolate)
      .SetMethod("onBeforeRequest",
                 &BrowserShellWebRequest::SetOnBeforeRequestListener);
}

const gin::WrapperInfo* BrowserShellWebRequest::wrapper_info() const {
  return &kWrapperInfo;
}
