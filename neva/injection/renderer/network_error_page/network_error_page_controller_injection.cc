...
void NetworkErrorPageControllerInjection::Install(blink::WebLocalFrame* frame) {
  ...
  auto* controller =
      cppgc::MakeGarbageCollected<NetworkErrorPageControllerInjection>(
          isolate->GetCppHeap()->GetAllocationHandle());

  v8::Local<v8::Object> controller_wrapper;
  if (!controller->GetWrapper(isolate).ToLocal(&controller_wrapper)) {
    return;
  }
...
const gin::WrapperInfo* NetworkErrorPageControllerInjection::wrapper_info()
    const {
  return &kWrapperInfo;
}
