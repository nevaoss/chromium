...
void WebOSSystemInjection::BuildExtraObjects(v8::Local<v8::Object> obj,
  ...
  auto* window_obj = cppgc::MakeGarbageCollected<WindowInjection>(
      isolate->GetCppHeap()->GetAllocationHandle(), this);

  v8::Local<v8::Object> window_obj_wrapper;
  if (!window_obj->GetWrapper(isolate).ToLocal(&window_obj_wrapper)) {
    return;
  }

  obj->Set(context, gin::StringToV8(isolate, "window"), window_obj_wrapper)
      .Check();

  // Build webOSSystem.cursor
  auto* cursor_obj = cppgc::MakeGarbageCollected<CursorInjection>(
      isolate->GetCppHeap()->GetAllocationHandle(), this);

  v8::Local<v8::Object> cursor_obj_wrapper;
  if (!cursor_obj->GetWrapper(isolate).ToLocal(&cursor_obj_wrapper)) {
    return;
  }

  obj->Set(context, gin::StringToV8(isolate, "cursor"), cursor_obj_wrapper)
      .Check();
...
void WebOSSystemInjection::Install(blink::WebLocalFrame* frame) {
  ...
  auto* webossystem_injection_handle =
      cppgc::MakeGarbageCollected<WebOSSystemInjection>(
          isolate->GetCppHeap()->GetAllocationHandle(), frame);

  v8::Local<v8::Object> webossystem_injection_handle_wrapper;
  if (!webossystem_injection_handle->GetWrapper(isolate).ToLocal(
          &webossystem_injection_handle_wrapper)) {
    return;
  }

  global
      ->Set(frame->MainWorldScriptContext(),
            gin::StringToV8(isolate, "webOSSystem"),
            webossystem_injection_handle_wrapper)
      .Check();
  ...
  if (webossystem_injection_handle->GetWrapper(isolate).ToLocal(
          &local_webossystem)) {
    webossystem_injection_handle->BuildExtraObjects(local_webossystem, isolate,
                                                    context);
  }
}
...
const gin::WrapperInfo* WebOSSystemInjection::wrapper_info() const {
  return &kWrapperInfo;
}
