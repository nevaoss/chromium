...
void InjectionEventsEmitterBase::DoAddEventListener(gin::Arguments* args,
  ...
  v8::Isolate* isolate = args->isolate();
  it->second.push_back({v8::TracedReference<v8::Function>(isolate, func),
                        std::move(is_once_listener)});
}
...
void InjectionEventsEmitterBase::Trace(cppgc::Visitor* visitor) const {
  for (const auto& listener : listeners_) {
    for (const auto& handler : listener.second) {
      visitor->Trace(handler.first);
    }
  }
}
