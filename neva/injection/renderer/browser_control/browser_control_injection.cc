...
gin::ObjectTemplateBuilder BrowserControlInjection::GetObjectTemplateBuilder(
    v8::Isolate* isolate) {
  return gin::Wrappable<BrowserControlInjection>::GetObjectTemplateBuilder(
             isolate)
...
v8::MaybeLocal<v8::Object> BrowserControlInjection::CreateObject(
  ...
  auto* browser_control = cppgc::MakeGarbageCollected<BrowserControlInjection>(
      isolate->GetCppHeap()->GetAllocationHandle(), frame);

  v8::MaybeLocal<v8::Object> maybe_browser_control_wrapper =
      browser_control->GetWrapper(isolate);

  v8::Local<v8::Object> browser_control_wrapper;
  if (!maybe_browser_control_wrapper.ToLocal(&browser_control_wrapper)) {
  ...
  parent
      ->Set(frame->MainWorldScriptContext(),
            gin::StringToV8(isolate, "browser_control"),
            browser_control_wrapper)
      .Check();

  return maybe_browser_control_wrapper;
}

const gin::WrapperInfo* BrowserControlInjection::wrapper_info() const {
  return &kWrapperInfo;
}
