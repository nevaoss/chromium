...
class PalServiceImpl : public mojom::PalService {
 ...
 private:
  void BindLaunchPoint(
      mojo::PendingReceiver<mojom::LaunchPoint> receiver) override;
  ...
  std::unique_ptr<pal::LaunchPointImpl> launchpoint_impl_;
...
void PalServiceImpl::BindLaunchPoint(
    mojo::PendingReceiver<mojom::LaunchPoint> receiver) {
  if (!launchpoint_impl_) {
    launchpoint_impl_ = std::make_unique<LaunchPointImpl>();
  }
  launchpoint_impl_->AddBinding(std::move(receiver));
}
