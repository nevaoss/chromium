...
void PageViewImpl::RegisterClient(
    mojo::PendingAssociatedRemote<mojom::PageViewClient> remote) {
...
void PageViewImpl::SyncBounds(SyncBoundsCallback callback) {
  auto bounds = page_view_->GetBounds();
  std::move(callback).Run(browser_shell::mojom::Rect::New(
      bounds.x(), bounds.y(), bounds.width(), bounds.height()));
}

void PageViewImpl::SetBounds(browser_shell::mojom::RectPtr bounds) {
  page_view_->SetBounds(bounds->x, bounds->y, bounds->width, bounds->height);
}
...
void PageViewImpl::OnVisibilityChanged(
  ...
  for (auto& remote : remotes_)
    remote->VisibilityChanged(visible, static_cast<unsigned int>(reason));
}
