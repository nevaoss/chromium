...
void WebView::CreateRenderView() {
  if (web_contents_) {
    ...
    if (!rnd_frame_host->IsRenderFrameLive()) {
      ...
      webcontents_impl->CreateRenderViewForRenderManager(rnd_view_host,
                                                         std::nullopt, nullptr);
    }
  }
}
...
// TODO(neva): This method is deprecated by using FreezingPolicy.
// Remove the method when wam_demo doesn't invoke it anymore.
