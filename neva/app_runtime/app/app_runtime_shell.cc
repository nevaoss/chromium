...
ShellWindow* Shell::CreateMainWindow(std::string url,
  ...
  page_view->SetBounds(main_window_->bounds());
  main_window_->AddObserver(this);
  main_window_->SetMainPageView(std::move(page_view));
  main_window_->GetMainPageContents()->SetFocus();
...
std::string Shell::GetAcceptedLanguages() const {
  auto* page_contents = main_window_->GetMainPageContents();
  if (page_contents) {
