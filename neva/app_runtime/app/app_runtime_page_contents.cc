...
void PageContents::ClearData(const std::string& clear_options,
  ...
  std::optional<base::Value> options = base::JSONReader::Read(clear_options);
...
// TODO(neva): Revise solution to support multiple devices in MediaStreamRequest
  if (request.audio_type ==
    ...
    devices.audio_device = blink::MediaStreamDevice(
        request.audio_type, request.requested_audio_device_ids.front(), "");
  }
  if (request.video_type ==
      blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE) {
    devices.video_device = blink::MediaStreamDevice(
        request.video_type, request.requested_video_device_ids.front(), "");
  }
