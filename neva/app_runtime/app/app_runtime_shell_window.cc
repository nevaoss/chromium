...
PageView* ShellWindow::GetMainPageView() const {
  return main_page_view_.get();
}

PageContents* ShellWindow::GetMainPageContents() const {
  return main_page_view_ ? main_page_view_->GetPageContents() : nullptr;
}

std::unique_ptr<PageView> ShellWindow::SetMainPageView(
    std::unique_ptr<PageView> page_view) {
  if (!page_view || !page_view->IsMain()) {
    return std::unique_ptr<PageView>();
  }

  DeleteAllChildViews();

  auto previous_main_page_view = std::move(main_page_view_);
  if (previous_main_page_view.get()) {
    previous_main_page_view->SetParentShellWindow(nullptr);
  }

  main_page_view_ = std::move(page_view);
  main_page_view_->SetParentShellWindow(this);

  AddChildView(main_page_view_->GetView());

  auto* root_view = GetWidget()->GetRootView();
  if (root_view) {
    root_view->GetLayoutManager()->Layout(root_view);
  }

  return previous_main_page_view;
}

std::unique_ptr<PageView> ShellWindow::RemoveMainPageView() {
  main_page_view_->SetParentShellWindow(nullptr);
  DeleteAllChildViews();
  return std::move(main_page_view_);
}
...
void ShellWindow::CloseNow() {
  if (!is_closing_) {
    is_closing_ = true;
    main_page_view_->CloseIfMain();
    if (window_widget_)
      window_widget_->CloseNow();
  }
}

PageView* ShellWindow::AddChildPageView(std::unique_ptr<PageView> page_view) {
  if (!page_view || page_view->IsMain()) {
    return nullptr;
  }
  ...
  if (inserted.second) {
    ...
    AddChildView(inserted.first->first->GetView());
    inserted.first->first->SetParentShellWindow(this);
    return inserted.first->first;
  }
  return nullptr;
}

std::unique_ptr<PageView> ShellWindow::RemoveChildPageView(
    PageView* page_view) {
  ...
  detached->SetParentShellWindow(nullptr);
  RemoveChildView(page_view->GetView());
  map_view_to_page_view_.erase(page_view->GetView());
  return detached;
}

std::list<PageView*> ShellWindow::GetChildPageViews() const {
...
void ShellWindow::DeleteAllChildViews() {
  for (const auto& child : map_view_to_page_view_) {
    RemoveChildView(child.first);
  }
  ...
  if (main_page_view_) {
    RemoveChildView(main_page_view_->GetView());
  }
}

void ShellWindow::BringToFront(PageView* child_page_view) {
  NEVA_DCHECK(child_page_view != nullptr);
  if (main_page_view_.get() == child_page_view ||
      child_page_views_.count(child_page_view) > 0) {
    auto* child_view = child_page_view->GetView();
    ReorderChildView(child_view, children().size());
  }
}

void ShellWindow::SendToBack(PageView* child_page_view) {
  NEVA_DCHECK(child_page_view != nullptr);
  if (main_page_view_.get() == child_page_view ||
      child_page_views_.count(child_page_view) > 0) {
    auto* child_view = child_page_view->GetView();
    ReorderChildView(child_view, 0);
  }
}
...
void ShellWindow::OnBoundsChanged(const gfx::Rect&) {
...
void ShellWindow::Init(const CreateParams& params) {
  ...
  std::unique_ptr<views::DesktopNativeCursorManager> native_cursor_manager =
      std::make_unique<views::DesktopNativeCursorManager>();
