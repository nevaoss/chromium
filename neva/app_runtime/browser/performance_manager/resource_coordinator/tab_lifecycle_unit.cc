...
class TabLifecycleUnitExternalImpl : public TabLifecycleUnitExternal {
 public:
  ...
  bool DiscardTab(LifecycleUnitDiscardReason reason,
                  base::ByteCount memory_footprint_estimate) override {
...
bool TabLifecycleUnitSource::TabLifecycleUnit::FinishDiscard(
    LifecycleUnitDiscardReason discard_reason,
    base::ByteCount tab_memory_footprint_estimate) {
  ...
  if (webview) {
    ...
    // Send the notification to WebContentsObservers that the old content is
    // about to be discarded and replaced with `null_contents`.
    old_contents->AboutToBeDiscarded(null_contents.get());

    // Copy over the state from the navigation controller to preserve the
    // back/forward history and to continue to display the correct
    // title/favicon.
    //
    // Set |needs_reload| to false so that the tab is not automatically reloaded
    // when activated. If it was true, there would be an immediate reload when
    // the active tab of a non-visible window is discarded. SetFocused() will
    // take care of reloading the tab when it becomes active in a focused
    // window.
    ...
    neva_app_runtime::WebViewMap::GetInstance()->RemoveWebContents(
        old_contents);
    ...
    neva_app_runtime::PageContents* page_contents =
        neva_app_runtime::PageContents::From(old_contents);
    if (page_contents) {
      page_contents->Deactivate(true);
    } else {
      LOG(ERROR) << "Cannot find the WebContents " << old_contents
                 << " from WebView or PageContents";
      return false;
    }
  }
...
bool TabLifecycleUnitSource::TabLifecycleUnit::Discard(
  ...
  if (FinishDiscard(reason, tab_memory_footprint_estimate)) {
