...
bool IsAllowedToLoadExtensionsIn(std::string_view partition,
                                 bool off_the_record) {
  ...
  if (cmd_line->HasSwitch(kAllowLoadExtensionsIn)) {
    ...
    for (auto& it : allowed_list) {
      StoragePartitionInfo allowed_item = ParseStoragePartitionSpec(it);
...
// static
AppRuntimeBrowserContext* AppRuntimeBrowserContext::From(
    std::string_view partition,
    bool off_the_record) {
  std::string_view name = partition;
  ...
  const bool allow_to_load_extensions =
      IsAllowedToLoadExtensionsIn(partition, off_the_record);

  auto* context = new AppRuntimeBrowserContext(name, off_the_record,
                                               allow_to_load_extensions);
  registry.insert(std::make_pair(
      std::string(name), std::unique_ptr<AppRuntimeBrowserContext>(context)));
  return context;
}

// static
AppRuntimeBrowserContext* AppRuntimeBrowserContext::From(
    const StoragePartitionInfo& info) {
  return AppRuntimeBrowserContext::From(info.name, info.off_the_record);
}
...
AppRuntimeBrowserContext::GetClientHintsControllerDelegate() {
  if (!hints_delegate_) {
    hints_delegate_ = std::make_unique<neva_user_agent::ClientHints>();
  }
