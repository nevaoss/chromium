...
void WebContentsItem::WebContentsDestroyed() {
  ...
  for (auto* observer : observers) {
    observer->WebContentsDestroyed(web_contents());
  }
  delete this;
}
...
void WebContentsItem::RenderFrameDeleted(
  ...
  auto& observers = WebContentsMap::GetInstance()->GetEventObservers();
  for (auto* observer : observers) {
    observer->RenderFrameDeleted(web_contents(), render_frame_host);
  }
}
...
void WebContentsItem::DOMContentLoaded(
  ...
  auto& observers = WebContentsMap::GetInstance()->GetEventObservers();
  for (auto* observer : observers) {
    observer->DOMContentLoaded(web_contents(), render_frame_host);
  }
}
...
void WebContentsItem::DidFinishLoad(content::RenderFrameHost* render_frame_host,
  ...
  auto& observers = WebContentsMap::GetInstance()->GetEventObservers();
  for (auto* observer : observers) {
    observer->DidFinishLoad(web_contents(), render_frame_host, validated_url);
  }
}
...
void WebContentsItem::DidFailLoad(content::RenderFrameHost* render_frame_host,
  ...
  auto& observers = WebContentsMap::GetInstance()->GetEventObservers();
  for (auto* observer : observers) {
    observer->DidFailLoad(web_contents(), render_frame_host, validated_url,
                          error_code);
  }
}
...
void WebContentsItem::DidOpenRequestedURL(
  ...
  for (auto* observer : observers) {
    observer->DidOpenRequestedURL(
        web_contents(), new_contents, source_render_frame_host, url, referrer,
        disposition, transition, started_from_context_menu, renderer_initiated);
  }
}
