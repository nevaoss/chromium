...
// Security: CastComponent::OnNavigationStateChanged() posts the
// NamedMessagePortConnector control port with target_origin="*" on EVERY
// main-document load, and the connector / Agent binding scripts are registered
// with origins={"*"}. So if a Cast app top-level navigates to a foreign origin,
// that origin inherits a live channel to chromium.cast.ApiBindings.Connect().
//
// This test PASSES on a vulnerable build (it observes the leak); apply the
// origin-check fix and it will time out / fail instead.
TEST_F(CastRunnerIntegrationTest, CrossOriginInheritsApiBindingsControlPort) {
  // Second server == different port == different web origin. This is the
  // "attacker" origin that the registered Cast app will navigate to.
  net::EmbeddedTestServer attacker_server;
  attacker_server.ServeFilesFromSourceDirectory(
      "fuchsia_web/runners/cast/testdata");
  ASSERT_TRUE(attacker_server.Start());
  const GURL attacker_url =
      attacker_server.GetURL("/cross_origin_attacker.html");

  // The registered Cast app's web_url lives on test_server() (origin A) and
  // simply top-level navigates to the attacker URL (origin B). cast_runner
  // installs no NavigationPolicyProvider, so nothing blocks this.
  const GURL app_url = test_server().GetURL(
      std::string("/cross_origin_redirect.html?") + attacker_url.spec());
  ASSERT_NE(url::Origin::Create(app_url), url::Origin::Create(attacker_url));

  TestCastComponent component(test_realm_services());
  app_config_manager().AddApp(kTestAppId, app_url);

  // No Agent binding scripts are needed: the attacker page either uses the
  // wildcard-injected cast.__platform__.PortConnector shim, or listens for the
  // wildcard window.onmessage("cast.master.connect") handshake directly.
  component.StartCastComponent(base::StrCat({"cast:", kTestAppId}));

  // The attacker page (origin B) requests a backend named "stolen_backend".
  // NamedMessagePortConnector::OnMessage -> ApiBindingsClient::OnPortConnected
  // forward it to chromium.cast.ApiBindings.Connect() with no origin check and
  // no port-name allowlist. Since the fix is applied, the connection should
  // be blocked and we expect it to time out.
  fuchsia::web::MessagePortPtr stolen =
      component.api_bindings()
          .RunAndReturnConnectedPort("stolen_backend", base::Seconds(2))
          .Bind();
  EXPECT_FALSE(stolen);
}

TEST_F(CastRunnerIntegrationTest,
       SameOriginRedirectInheritsApiBindingsControlPort) {
  // Both pages live on test_server() == same web origin.
  const GURL target_url = test_server().GetURL("/cross_origin_attacker.html");
  const GURL app_url = test_server().GetURL(
      std::string("/cross_origin_redirect.html?") + target_url.spec());
  ASSERT_EQ(url::Origin::Create(app_url), url::Origin::Create(target_url));
  ...
  // Since they are same-origin, the connection should succeed and not time out.
  fuchsia::web::MessagePortPtr stolen =
      component.api_bindings()
          .RunAndReturnConnectedPort("stolen_backend")
          .Bind();
  ASSERT_TRUE(stolen);

  // Round-trip a message to prove it works.
  fuchsia::web::WebMessage probe;
  probe.set_data(base::MemBufferFromString("ping", "probe"));
  stolen->PostMessage(std::move(probe),
                      [](fuchsia::web::MessagePort_PostMessage_Result r) {
                        EXPECT_TRUE(r.is_response());
                      });

  base::test::TestFuture<fuchsia::web::WebMessage> reply;
  stolen->ReceiveMessage(CallbackToFitFunction(reply.GetCallback()));
  ASSERT_TRUE(reply.Wait());
  const std::string reply_text =
      base::StringFromMemBuffer(reply.Get().data()).value_or(std::string());

  const std::string target_origin = url::Origin::Create(target_url).Serialize();
  EXPECT_EQ(reply_text, "attacker[" + target_origin + "]:ping");
}
