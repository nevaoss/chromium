...
// which was removed in the upstream CL http://crrev.com/c/6182387.
//
...
#include "third_party/blink/renderer/platform/image-decoders/avif/neva_avif_image_decoder.h"
...
NevaAVIFImageDecoder::NevaAVIFImageDecoder(
...
NevaAVIFImageDecoder::~NevaAVIFImageDecoder() = default;

String NevaAVIFImageDecoder::FilenameExtension() const {
  return "avif";
}

const AtomicString& NevaAVIFImageDecoder::MimeType() const {
  DEFINE_STATIC_LOCAL(const AtomicString, avif_mime_type, ("image/avif"));
  return avif_mime_type;
}

bool NevaAVIFImageDecoder::ImageIsHighBitDepth() {
  return bit_depth_ > 8;
}

void NevaAVIFImageDecoder::OnSetData(scoped_refptr<SegmentReader> data) {
...
cc::YUVSubsampling NevaAVIFImageDecoder::GetYUVSubsampling() const {
...
gfx::Size NevaAVIFImageDecoder::DecodedYUVSize(cc::YUVIndex index) const {
...
wtf_size_t NevaAVIFImageDecoder::DecodedYUVWidthBytes(
...
SkYUVColorSpace NevaAVIFImageDecoder::GetYUVColorSpace() const {
  DCHECK(CanDecodeToYUV());
  DCHECK_NE(yuv_color_space_, SkYUVColorSpace::kIdentity_SkYUVColorSpace);
  return yuv_color_space_;
}

uint8_t NevaAVIFImageDecoder::GetYUVBitDepth() const {
  DCHECK(CanDecodeToYUV());
  return bit_depth_;
}

void NevaAVIFImageDecoder::DecodeToYUV() {
...
int NevaAVIFImageDecoder::RepetitionCount() const {
...
bool NevaAVIFImageDecoder::FrameIsReceivedAtIndex(wtf_size_t index) const {
...
std::optional<base::TimeDelta> NevaAVIFImageDecoder::FrameTimestampAtIndex(
...
base::TimeDelta NevaAVIFImageDecoder::FrameDurationAtIndex(
    wtf_size_t index) const {
  return index < frame_buffer_cache_.size()
...
bool NevaAVIFImageDecoder::ImageHasBothStillAndAnimatedSubImages() const {
...
// static
bool NevaAVIFImageDecoder::MatchesAVIFSignature(
...
gfx::ColorSpace NevaAVIFImageDecoder::GetColorSpaceForTesting() const {
  const auto* image = GetDecoderImage();
  CHECK(image);
  return GetColorSpace(image);
}

void NevaAVIFImageDecoder::ParseMetadata() {
  if (!UpdateDemuxer()) {
    SetFailed();
  }
}

void NevaAVIFImageDecoder::DecodeSize() {
  ParseMetadata();
}

wtf_size_t NevaAVIFImageDecoder::DecodeFrameCount() {
  if (!Failed()) {
    ParseMetadata();
  }
...
void NevaAVIFImageDecoder::InitializeNewFrame(wtf_size_t index) {
...
void NevaAVIFImageDecoder::Decode(wtf_size_t index) {
  if (Failed()) {
    return;
  }
...
bool NevaAVIFImageDecoder::CanReusePreviousFrameBuffer(wtf_size_t index) const {
...
// static
avifResult NevaAVIFImageDecoder::ReadFromSegmentReader(avifIO* io,
                                                       uint32_t read_flags,
                                                       uint64_t offset,
                                                       size_t size,
                                                       avifROData* out) {
...
bool NevaAVIFImageDecoder::UpdateDemuxer() {
  DCHECK(!Failed());
  if (IsDecodedSizeAvailable()) {
    return true;
  }
...
avifResult NevaAVIFImageDecoder::DecodeImage(wtf_size_t index) {
...
void NevaAVIFImageDecoder::CropDecodedImage() {
...
bool NevaAVIFImageDecoder::RenderImage(const avifImage* image,
                                       int from_row,
                                       int* to_row,
                                       ImageFrame* buffer) {
...
void NevaAVIFImageDecoder::ColorCorrectImage(int from_row,
                                             int to_row,
                                             ImageFrame* buffer) {
...
bool NevaAVIFImageDecoder::GetGainmapInfoAndData(
...
avifImage* NevaAVIFImageDecoder::GetDecoderImage() const {
  if (aux_image_ == cc::AuxImage::kGainmap) {
    if (!decoder_->image->gainMap) {
...
NevaAVIFImageDecoder::AvifIOData::AvifIOData() = default;
NevaAVIFImageDecoder::AvifIOData::AvifIOData(
