...
void InputMethodAuraLinuxNeva::SendFakePressKeyEvent(uint16_t key_value) const {
  ui::KeyEvent char_event(ui::EventType::kKeyPressed, ui::VKEY_UNKNOWN,
                          ui::DomCode::NONE, ui::EF_IME_FABRICATED_KEY,
                          ui::DomKey::FromCharacter(key_value),
                          base::TimeTicks(), false);

  std::ignore = DispatchKeyEventPostIME(&char_event);
...
void InputMethodAuraLinuxNeva::CancelComposition(
  ...
  if (IsPasswordInputFieldFocused() && !composition_.text.empty()) {
    SendFakePressKeyEvent(static_cast<char16_t>(composition_.text[0]));
  }
